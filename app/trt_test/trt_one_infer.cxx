...
void testBase(){
    char msg[1024]{};
    std::string cls_model_pth{R"(E:\Pretrained_models\YOLOv11-cls\yolo11n-cls-fp16.engine)"};
    Classify cls_infer(cls_model_pth.c_str(), msg);

    std::string det_model_pth{R"(E:\Pretrained_models\YOLOv11\yolo11n-fp16.engine)"};
    Detection det_infer(det_model_pth.c_str(), msg);

    std::string seg_model_pth{R"(E:\Pretrained_models\YOLOv11-seg\yolo11n-fp16.engine)"};
...
void testInterface(){
    ...
    auto cls_res = doInferenceByImgPath(cls_infer, img_pth.c_str(), nullptr, 0.3f, det_num, msg);
    std::cout << static_cast<CLS_RES*>(cls_res)[0].get_info() << std::endl;

    auto cls_res2 = doInferenceByImgPath(cls_infer2, img_pth2.c_str(), nullptr, 0.3f, det_num, msg);
    std::cout << static_cast<CLS_RES*>(cls_res2)[0].get_info() << std::endl;

    auto det_res = doInferenceByImgPath(det_infer, img_pth.c_str(), nullptr, 0.3f, det_num, msg);
    std::cout << static_cast<DET_RES*>(det_res)[0].get_info() << std::endl;

    auto seg_res = doInferenceByImgPath(seg_infer, img_pth.c_str(), nullptr, 0.3f, det_num, msg);
